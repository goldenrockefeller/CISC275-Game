...
public class GameState {
	...
	Shooter gameShooter;
	final static int frameWidth = Launcher.frameWidth;
    final static int frameHeight = Launcher.frameHeight;
	...
	public void update()
	{
		//If first projectile, or food that has been eaten, or trash that has stopped, load next projectile
		if (projectile == null  || !projectile.isVisible || projectile.stopped) {
			loadProjectile(gameShooter);
			getProjectile().foodFlag++;
			getProjectile().trashFlag++;
		}
		...
		for (GameObject gameObject : gameObjectCollection)
		{
			...
			if (gameObject instanceof trashcanObject) {
				if (((trashcanObject) gameObject).checkCollision()) {
				}
			}
		}
		if (hitb && buffb <= 0) {
			this.add(new BlueCrab(frameWidth/2,frameHeight/2,4,2,this));
			blueCount++;
			buffb = 300;
			buffm = 300;
		}
		if (hitm && buffm <= 0) {
			this.add(new MittenCrab(frameWidth/2,frameHeight/2,4,2,this));
			mittenCount++;
			buffm = 300;
			buffb = 300;
		}
	...
	public void loadProjectile(Shooter gameShooter) {
		...
		if (randProj < 5) {
			setProjectile(new Food(frameWidth/2,frameHeight-200,0,0,this));
		}
		else {
			setProjectile(new Trash(frameWidth/2,frameHeight-200,0,0,this));
		}
