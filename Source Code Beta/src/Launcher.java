...
public class Launcher {
	...
	public static final int frameWidth = (int) screenSize.getWidth();
	public static final int frameHeight = (int) screenSize.getHeight();
	...
	public static void setupGameState (GameState gameState)
	{
		...
		gameState.add(new MittenCrab(frameWidth/2,frameHeight/2,4,2,gameState));
		gameState.add(new BlueCrab(frameWidth/2,frameHeight/2,6,2,gameState));
		//gameState.add(new TestObject(480,850,0,0,gameState));
		//gameState.add(new mytestObject(32,64,0,0,gameState));
		gameState.add(new powerbarFrame(frameWidth/2 - 200, frameHeight - 200, 0, 0, gameState));
		gameState.add(new powerbarObject(frameWidth/2 - 200,frameHeight - 198,0,0,gameState));
		//gameState.add(new Food(480,700,0,0,gameState));
		gameState.add(new trashcanObject(200,120,0,0,gameState));
		gameState.add(new trashcanObject(frameWidth / 2,120,0,0,gameState));
		gameState.add(new trashcanObject(frameWidth - 200,120,0,0,gameState));
		gameState.setProjectile(new Food(frameWidth/2,frameHeight-200,0,0,gameState));
