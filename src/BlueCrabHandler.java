...
public class BlueCrabHandler extends Handler{
	...
	BlueCrabHandler(GameScreen gameScreen,GameState gameState) {
		...
		try{
			image[0] = ImageIO.read(new File("img/blueclaw/bluecrab_0.png")).getScaledInstance(scaleX(Crab.width), scaleY(Crab.height), Image.SCALE_FAST);
			image[1] = ImageIO.read(new File("img/blueclaw/bluecrab_1.png")).getScaledInstance(scaleX(Crab.width), scaleY(Crab.height), Image.SCALE_FAST);
			image[2] = ImageIO.read(new File("img/blueclaw/bluecrab_2.png")).getScaledInstance(scaleX(Crab.width), scaleY(Crab.height), Image.SCALE_FAST);
		}
		catch(IOException e)
		{
